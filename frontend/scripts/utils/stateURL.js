...
const appStateToURLParams = state => {
  ...
  return {
    selectedContextId: state.app.selectedContextId,
...
const URLParamsToAppState = params => {
  const appReducerState = removeEmptyParams({
    selectedContextId: params.selectedContextId
  });
...
export const rehydrateAppStateFromToolURL = (action, next) => {
  ...
  if (urlState) {
    next({
      ...
      payload: URLParamsToAppState(urlState)
    });
  }
  return next(action);
};
...
export const toolUrlStateMiddleware = store => next => action => {
  ...
  const result = rehydrateAppStateFromToolURL(action, next);
